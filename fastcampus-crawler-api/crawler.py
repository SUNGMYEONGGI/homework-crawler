...
class FastCampusLMSCrawler:
    ...
    def _collect_data_item(self, student_name, blog_link):
        ...
        self._add_log(f"데이터 수집: 이름={student_name}, 링크={blog_link[:30] if blog_link else ''}...")
    ...
    async def crawl_exam_data_async(self, exam_id, progress_callback, log_callback):
        ...
        for i in range(total_count):
            ...
            current_progress_val = 0.4 + (i / total_count) * 0.5  # 40%~90% 범위
            ...
            try:
                ...
                try:
                    ...
                    await update_log_and_progress(current_progress_val, f"블로그 링크/내용 수집: {blog_link[:50] if blog_link else ''}...")
                    ...
                    await update_log_and_progress(current_progress_val, f"{student_name}: 블로그 링크 수집 중 Timeout (항목 없음 가능성)")
                except Exception as e_blog:
                    await update_log_and_progress(current_progress_val, f"{student_name}: 블로그 링크 수집 중 오류 - {e_blog}")
                ...
                try:
                    ...
                    await update_log_and_progress(current_progress_val, f"{student_name}: 두 번째 모달 닫기 중 오류 - {e_modal2}")
                ...
                if i < total_count - 1:
                    try:
                        ...
                        if self.driver.find_elements(By.XPATH, next_button_xpath_err + "[not(@disabled)]"):
                            ...
                            await update_log_and_progress(current_progress_val, "다음 버튼 비활성화 또는 없음. 중단.")
                            break
